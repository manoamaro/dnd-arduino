...
void AdvancedUI::printDetails(OpResultDetail *details, Adafruit_SSD1306 *display)
{
    for (int i = 0; i < OP_RESULT_MAX_DETAILS; i++) {
        ...
        case OP_RESULT_DETAIL_TYPE_CONST:
            display->print(detail.getValue(), DEC);
            break;
...
void AdvancedUI::render(Adafruit_SSD1306 *display)
{
    ...
    else if (state == UI_SHOW_RESULT_STATE)
    {
        ...
        display->print(this->result.getResult(), DEC);
        display->setTextSize(1);
        display->setCursor(0, 0);
        printDetails(this->result.getDetails(), display);
    }
    display->display();
}
...
void AdvancedUI::right(bool longPress)
{
    ...
    else
    {
        if (this->state == UI_CONST_DICE_STATE)
        {
            ...
            {
                this->currConstOp = new ConstOp(10);
